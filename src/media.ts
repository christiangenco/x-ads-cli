...
export async function uploadMedia(filePath: string): Promise<string> {
  ...
  if (isVideo(mimeType) && finalizeResponse.processing_info) {
    ...
    while (processingInfo && processingInfo.state !== "succeeded" && processingInfo.state !== "failed") {
      ...
      console.log(`  Processing video... waiting ${waitSeconds}s (state: ${processingInfo.state})`);
  ...
  // Return media_key if available, otherwise media_id_string
  const mediaKey = finalizeResponse.media_key || mediaId;
  return mediaKey;
}
